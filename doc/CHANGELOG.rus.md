...
- Исправлена сборка с обновлённым компилятором Visual Studio (@ForserX)
...
- Исправлено некорректное изменение метода масштабирования избражения (@Hozar2002)
...
- Реализован алгоритм AStar (@isoboleskiy)
- Реализован Agent Manager Planar без Goap (@isoboleskiy)
- Удалены не синглтонный `CStringTable` объекты (@Drombeys)
...
- Упрощенна настройка игрового курсора (@Drombeys, @Hozar2002)
...
- Добавлена оптимизация макс.высоких теней от солнца (@ForserX, @Zagolski)
- Добавлен учет тумана для деталей на статического освещения (@Drombeys)
...
- Реструктурированный xrRender (@v2v3v4)
...
- Добавлен хак для скриптованных диалогов (@Drombeys)
...
- Исправлен краш при переключение атаки контроллера (@Drombeys, @ForserX)
- Исправлен краш при создании дефолтного сервера (@kola2451)
- Исправлен краш при открытии ПДА в мультиплеере (@kola2451)
- Исправлен краш с кастомной моделью hud игрока (@Hozar2002)
...
- Восстановлено использование станционарного пулемета (@St4lker0k765, @Alundaio)
...
- Добавлен лэйаут для `FreeMP` (@kola2451)
...
- Добавлена ультра настройка графики (@Drombeys)
- Изменена директория настроек игры по умолчанию (@St4lker0k765, @Drombeys)
- Декомпозирован XML `ui_icons_npc` (@Drombeys)
...
- Удалены скриптованные вызовы для отключения тени актера в кат-сценах (@Drombeys, @Hozar2002)
- Удалены скриптованные вызовы метода `is_god_mode` в Lua (@Drombeys)
- Отменены изменения в менеджере погоды (@Drombeys)
- Упрощена адаптация скриптов (@Drombeys)
- Перенесены изменения оригинальных файлов в конфигурацию `mod_system_ixray` (@Drombeys)
...
- Добавлена поддержка snd env в `Play in Editor` (@ForserX)
...
- Исправлена загрузка `*.dti` (@ForserX)
...
- Исправлен краш в методе `GetDrawText` (@Drombeys)
...
- Исправлен краш с метками при запуске `Play in Editor` (@VodkaNET)
...
- Реализован вид `Content Browser` (@ForserX)
...
- Рендер скелета в 1 вызов на отрисовку (@ForserX)
