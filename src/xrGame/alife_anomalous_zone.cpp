...
void CSE_ALifeAnomalousZone::spawn_artefacts()
{
	...
	using Weight = std::pair<shared_str,float>;
	using Weights = buffer_vector<Weight>;
	...
	for (u32 i = 0; i < n; ++i) 
	{
		...
		weights.push_back(std::make_pair(temp0, static_cast<float>(atof(temp1))));
	}

	u16 p = 0;
	u32 ii = 0;

	for (; ii<m_artefact_count; ++ii) 
	{
		float fProbability = randF(1.f), fSum = 0.f;
