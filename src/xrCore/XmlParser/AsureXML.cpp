...
static xr_string_map<xr_string, FS_FileSet> HashData;
...
FS_FileSet CXMLOverride::GetModifFiles(const char* Path, const char* File)
{
	...
	if (HashData.contains(ModifPathMask))
	{
		return HashData[ModifPathMask];
	}

	FS_FileSet& ModifyList = HashData[ModifPathMask];
