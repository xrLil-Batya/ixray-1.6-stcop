...
- Added data storage for XMLOverride system (@ForserX)
- Added external large atlas font (@ForserX)
...
- Maked `Continue` default button in error dialog (@Drombeys)
- Maked bones calculation thread-safe (@v2v3v4)
- Maked threadsafe `ISpatial` component (@ForserX)
...
- Added optional shader depth nearclip (@Hozar2002)
...
- Fixed `xform_calc` calls in `light` module (@v2v3v4)
...
- Implemented puddle support in r4 (@LVutner, @Hozar2002)
...
- Enabled block using items if hud animator active (@Shtrecker)
...
- Fixed eat low condition item (@Shtrecker)
...
- Fixed item transfer to dead NPC inventory (@ForserX)
- Fixed kill entity when GodMode is disabled (@Drombeys)
- Fixed loading thirst and sleepiness state (@ForserX)
- Fixed Lua exports (@ForserX, @Drombeys)
...
- Fixed sprint stutter after jump (@Drombeys, @ggGhosTt)
...
- Maked `putall_button` optional (@Drombeys)
- Reload particle xr when particle is playing (@mnelenpridumivat)
...
- Unlocked `PlayerHudOmfAdditional` parameter (@Drombeys)
...
- Added base code for exporting Level Editor scenes in one click (@ForserX)
...
- Added custom window title bar for Editors (@GladiatorVS)
...
- Added soft sounds sync support (@ForserX)
...
- Added temporary GPU Skinning for editors (@ForserX, @ugozapad, @Hozar2002)
...
- Fixed alpha in Thumbnail (@ForserX)
...
- Fixed exception when playing broken animations (@ForserX)
...
- Fixed particles reload when playing (@ForserX)
...
- Maked threadsafe for loading level (@ForserX)
...
- Replaced `Edit Mode` with D&D action (@ForserX)
